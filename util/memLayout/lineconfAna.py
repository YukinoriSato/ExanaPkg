...
def read_malloc_info(prog_name,mallocdpath):
	...
	while line:
		...
		if 0<= line.find("C"):
			...
			while -1==line.find("Return"):
				line = mfs.readline()

			v2=line.split(' ')
			if len(v2)<10:
				print "malloc.out contains a unparsable line: %s", line
				break
	                #print v2
			no = int(v2[3][1:-1])
			size = int(v2[7],16)
			...
			location = read_line_in_source_code(t1,prog_name)#fixpoint
			#print location
			##minfo = (no,mr.group(2),size,sintaddr,eintaddr,mr.group(5),t1,t2,rsl)			
				#if size >= 0x1000:
			memLocation.append((sintaddr,eintaddr,location))
			cnt+=1
...
def read_procmmap():
	...
	try:
		mfs = open("./procmmap.out","r")
	except IOError:
		print "procmmap.out can't open."
		sys.exit()
...
def checkMemLayout(start, end):
	...
	for i in range(len(memLocation)):
		...
		if(memLocation[i][0]<=start and (start<=memLocation[i][1])):
			...
			if name not in list:
				list.append(name)
				r=(name, (float)(size)/(memLocation[i][1]-memLocation[i][0])*100)
				rar.append(r)
	...
	ss=sorted(rar, key=itemgetter(1))

	for i in range(len(ss)):
		#f='%.1f' %(ss[1][i])
		if ss[i][0]=="Stack":
			aa="(%x, %d)" % (start, end-start)
			strname=strname+ss[i][0]+aa+" " 
		elif ss[i][0].find("malloc")!=-1:
			aa="(%x, %d)" % (memLocation[i][0], memLocation[i][1]-memLocation[i][0])
			strname=ss[i][0] 
		else:
			strname=strname
		break
...
def merge_lineconf(bin_name):
	...
	for i in range(num_check):
		...
		for j in range(len(new_lineconf_stat[i])):
			...
			for k in range(len(origin)):
				...
				if found2!=0:
					...
					if name=="" or memObj[n]=="":
						...
						attr.append(1)
						#print "  -> intra-array conflic
					elif endAddr-startAddr<= 8 or ea[n]-sa[n]<=8:
						attr.append(2)
						#print "  -> scalar conflict"						
			...
			#print "missPC   %x   L%d  total=%d  startAddr %x endAddr %x %s  loc=%s" % (new_lineconf_stat[i][j][0], new_lineconf_stat[i][j][1], new_lineconf_stat[i][j][2], startAddr, endAddr, name, missLocation[len(missLocation)-1])
			...
			## for memory object-relative profiling
			#print "%s #conflicts=%d  L%d loc=%s startAddr %x endAddr %x PC %x" % ( name, new_lineconf_stat[i][j][2], new_lineconf_stat[i][j][1],missLocation[len(missLocation)-1], startAddr, endAddr, new_lineconf_stat[i][j][0])
			tmp=[]
			for k in range(len(origin)):
				tt=(memObj[k], originSrc[k], originCnt[k], sa[k], ea[k], originPC[k])
				tmp.append(tt)
				#print "       evicted by  %s %s cnt %d sa %x ea %x " % (memObj[k], originSrc[k], originCnt[k], sa[k], ea[k], originPC[k])

			tmp2=sorted(tmp, key=itemgetter(2),reverse=True)
			ss=(name, new_lineconf_stat[i][j][2], new_lineconf_stat[i][j][1],missLocation[len(missLocation)-1], startAddr, endAddr, new_lineconf_stat[i][j][0], tmp2)
			lineconfNew.append(ss)
...
def main():
	...
	for i in range(len(mempatList)):
		...
		# when new ins comes, initiate tmp 
		if ins not in list:
			list.append(ins)
			tmp=[]
		...
		#print "tmp len=%d %s" % (len(tmp), tmp)

		# until the last element, append tmp if next ins is different
		if(i!=len(mempatList)-1):
			if(ins!=mempatList[i+1][0]):
				#print "if then append to newMemPatList %x  %x" % (ins, tmp[0][0])
				#cnt+=1
				newMemPatList.append(tmp)
		# when the last element, append tmp 
		else:
			newMemPatList.append(tmp)
			#print "else then append to newMemPatList %x  %s" % (ins, tmp[0][0])
	#print "cnt, len = %d %d %s" %(cnt, len(newMemPatList), newMemPatList)
	#sys.exit()

	"""			
	for i in range(len(newMemPatList)):
	 	print "%d" % len(newMemPatList[i])
		for j in range(len(newMemPatList[i])):
			print newMemPatList[i][j]
		#print newMemPatList[i][0]
	...
	for i in range(len(bin_reader.lineconf_stat)):
		print "missPC %x  L%d cnt=%d  %s" % bin_reader.lineconf_stat[i]
	"""
	...
	"""
	for i in range(len(lineconfNew)):
		print "%s cnt %d L%d PC %x" % (lineconfNew[i][0], lineconfNew[i][1], lineconfNew[i][2], lineconfNew[i][6])
		for j in range(len(lineconfNew[i][7])):
			tmp=lineconfNew[i][7][j]
			print "   %s  loc=%s  cnt=%d  originPC=%x" % (tmp[0],tmp[1],tmp[2],tmp[5])
	"""
	print"memory object-relative view"
	# for memory object-relative conflict profiling 
	lineconfNew.sort(key=itemgetter(0))
	lineconfNew2=[]
	newAna=[]
	list=[]
	for i in range(len(lineconfNew)):
		memObj=lineconfNew[i][0]
		
		if memObj not in list:
			list.append(memObj)
			tmp=[]
			tmp.append(lineconfNew[i])
		else:
			tmp.append(lineconfNew[i])
			#print "len=%d %s" % (len(tmp), tmp)

		if(i!=len(lineconfNew)-1):
			if(memObj!=lineconfNew[i+1][0]):
				lineconfNew2.append(tmp)
		else:
			lineconfNew2.append(tmp)


	for i in range(len(lineconfNew2)):
		PCs=[]
		for j in range(len(lineconfNew2[i])):
			if lineconfNew2[i][j][6] not in PCs:
				PCs.append(lineconfNew2[i][j][6])
		"""
		print "%s  len=%d  conflictMissPC=" % (lineconfNew2[i][0][0], len(lineconfNew2[i])),
		for j in range(len(PCs)):
			print "%x" % PCs[j],
		print ""
		"""
		cnt=[0,0,0]  # L1,L2,L3
		evictObj=[]  # name, cnt[3], originPC
		list2=[]
		for j in range(len(lineconfNew2[i])):
			elem=lineconfNew2[i][j]
			for k in range(len(lineconfNew2[i][j][7])):
				tmp=lineconfNew2[i][j][7][k]
				#print "      %s  loc=%s  cnt=%d  originPC=%x" % (tmp[0],tmp[1],tmp[2],tmp[5])
				cnt[elem[2]-1]+=tmp[2]
				if tmp[0] not in list2:
					tcnt=[0,0,0]
					pcList=[]
					#t=(lineconfNew2[i][j][2], tmp)
					name=tmp[0]
					tcnt[elem[2]-1]+=tmp[2]
					pcList.append(tmp[5])
					list2.append(tmp[0])
					...
					for id in range(len(evictObj)):
						if evictObj[id][0]==tmp[0]:
							break
					evictObj[id][1][elem[2]-1]+=tmp[2]
					if tmp[5] not in evictObj[id][2]:
						evictObj[id][2].append(tmp[5])
					#print "      update list ",
					#print evictObj[id]

		total=cnt[0]+cnt[1]+cnt[2]
		aa=(lineconfNew2[i][0][0], total, PCs, evictObj)
		newAna.append(aa)
		"""
		for j in range(len(evictObj)):
			print "    -->  %s  cnt=" % evictObj[j][0],
			for k in range(len(evictObj[j][1])):
				print "%d," % evictObj[j][1][k],
			print "originPC=",
			for k in range(len(evictObj[j][2])):
				print "%x " % evictObj[j][2][k],
			print ""
		"""
	newAna.sort( key=itemgetter(1),reverse=True)
	for i in range(len(newAna)):
		print "%s  total=%d  conflictMissPC=" % (newAna[i][0], newAna[i][1]),
		for j in range(len(newAna[i][2])):
			print "%x " % newAna[i][2][j],
		print ""
		evictObj=newAna[i][3]
		for j in range(len(evictObj)):
			print "    -->  %s  cnt=" % evictObj[j][0],
	...
	for i in range(len(lineconf_sort)):
		...
		for j in range(len(lineconf_sort[i][5])):
			...
			if origin[4]==0:
				...
				print "  [scalar] %x cnt=%d  memObj=%s loc=%s"%(origin[0], origin[1], origin[2], origin[3])
			else:
	...
	print ""
	print "-----------------------------------------------------------"
	print "            %12s  %12s   %12s   %12s   %12s" % ("sum", "inter-array","intra-array","scalar", "unknown")
	print "#conflict   %12d: %12d   %12d   %12d   %12d"%(sum, psum[0], psum[1], psum[2], psum[3])
	if sum > 0:
		print "%9s   %12s  %12.2f%1s  %12.2f%1s  %12.2f%1s  %12.2f%1s"%("Ratio    ", "", psum[0]*100.0/sum,"%", psum[1]*100.0/sum,"%", psum[2]*100.0/sum,"%",psum[3]*100.0/sum,"%")
	else:
		print "%9s   %12s  %12.2f%1s  %12.2f%1s  %12.2f%1s  %12.2f%1s"%("Ratio    ", "", 0,"%", 0,"%", 0,"%", 0,"%")
