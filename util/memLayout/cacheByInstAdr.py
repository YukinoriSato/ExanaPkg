...
def main():
	...
	try:
		#opts,args = getopt.getopt(sys.argv[1:],"b:p:vg:vh")
		# 引数が必要なときはコロンをつける
		opts,args = getopt.getopt(sys.argv[1:],'j:b:')		
	...
	for opt,a in opts:
		if opt == "-b":
			...
			print opt,a
			print "  % cacheByInstAdr.py [-b exefile] "
			print "        Options:   "
	...
	if json_flag==0:
		print "                                  L1                    L2                    L3           "
		print " instAddr    Est latency  #hit  #miss #confl    #hit  #miss #confl    #hit  #miss #confl   totalAccess"
	else:
		print "{"
		print "     \"header\": [\"instAddr\", \"Est latency\", \"L1 hit\", \"L1 miss\", \"L1 confl\", \"L2 hit\", \"L2 miss\", \"L2 confl\", \"L3 hit\", \"L3 miss\", \"L3 confl\", \"totalAccess\", \"src\"],"
		print "     \"data\":"
		print "     ["
	...
	for i in range( bin_reader.num_inst):
		...
		data=newlist2[i]
		L3miss=(data[4])
		L2miss=(data[4]+data[3])
		L1miss=(L2miss+data[2])
		location=""
		if(bin_flag):
			cmd_line = ["addr2line","-ie", bin_name,""]
			cmd_line[3] = str(hex(data[0]))
		...
		if json_flag==0:
			print " %8x   %10d  %6d %6d %6d  %6d %6d %6d  %6d %6d %6d   %11d  %s" % (data[0],  data[10], data[1], L1miss, data[5], data[2], L2miss, data[6], data[3], L3miss, data[7], data[8], location[len(location)-1])
		else:
			if bin_reader.num_inst-1!=i:
				print "        [\"%8x\", %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, \"%s\"]," % (data[0],  data[10], data[1], L1miss, data[5], data[2], L2miss, data[6], data[3], L3miss, data[7], data[8], location[len(location)-1])	
			else:
				print "        [\"%8x\", %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, \"%s\"]" % (data[0],  data[10], data[1], L1miss, data[5], data[2], L2miss, data[6], data[3], L3miss, data[7], data[8], location[len(location)-1])	
...
def read_section_header(prog_name,fname):
    ...
    while i < 7:
        ...
        i += 1
    #print line
    while 0 > line.find("Key to Flags:"):
        #print line
...
def mergeMempatAndCache(bin_name):
	...
	#for i in range( bin_reader.num_inst):
		...
		print "                       |     L1      |     L2      |     L3      |   DRAM "
		print " instAddr | Est lateny | miss% confl%| miss% confl%| miss% confl%| #accesses    "
		...
		print " %8x | %10d |%6.2f %6.2f|%6.2f %6.2f|%6.2f %6.2f| %9d " % (data[0],  data[10], f1, f2, f3, f4, f5, f6, L3miss)
