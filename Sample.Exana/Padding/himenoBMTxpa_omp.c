...
void mat_first_touch(Matrix* Mat,int l);
...
main(int argc, char *argv[])
{
  ...
  mat_first_touch(&p,0);
  mat_first_touch(&bnd,0);
  mat_first_touch(&wrk1,0);
  mat_first_touch(&wrk2,0);
  mat_first_touch(&a,0);
  mat_first_touch(&a,1);
  mat_first_touch(&a,2);
  mat_first_touch(&a,3);
  ...
  flop = (double)(kmax-2)*(double)(jmax-2)*(double)(imax-2)*34.0;
  if(cpu != 0.0)
    xmflops2= flop/cpu*1.e-6*nn;

  printf(" MFLOPS: %f time(s): %f %e\n\n",xmflops2,cpu,gosa);

  nn= (int)(target/(cpu/(float)nn));
#ifdef TEST
  nn = 10;
#endif
...
void
mat_first_touch(Matrix* Mat, int l)
{
  int i,j,k;
